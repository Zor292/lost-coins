  ...
  async execute(interaction, client) {
    ...
    if (interaction.isStringSelectMenu()) {
      ...
      if (interaction.customId.startsWith('buycoins_select_')) {
        ...
        // إنشاء طلب جديد مدته دقيقة واحدة
        ...
        setTimeout(async () => {
          try {
            ...
            if (p) {
              ...
              if (ch) {
                await ch.send({
                  content: `<@${userId}> انتهى وقت طلب شراء **${coinsRequested} لوست كوين**. استخدم \`-buycoins\` مجدداً للمحاولة.`
                }).catch(() => null);
              }
            }
          } catch {}
