...
function productListEmbed(items) {
  const embed = baseEmbed().setTitle('قائمة المنتجات');
  if (!items || items.length === 0) {
    embed.setDescription('لا توجد منتجات متاحة حاليًا.');
    return embed;
  }
  const lines = items.map(item => {
    const stock = item.quantity <= 0 ? '`نفد المخزون`' : `الكمية: \`${item.quantity}\``;
...
module.exports = { shopPanelEmbed, ticketOpenEmbed, productListEmbed, invoiceEmbed, logEmbed, baseEmbed };
